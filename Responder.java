...
public class Responder
{
     ...
     */
    public String generateResponse(HashSet<String> pregunta)
    {
        String answer = null;
        if(respuesta.get(pregunta) != null)
        {
            //answer = respuesta.get(pregunta);
            boolean palabraClave = false;
            Iterator it = pregunta.iterator();
            while(it.hasNext() && !palabraClave)
            {
                if(it.next() == respuesta.get(it))
                {
                    answer = respuesta.get(pregunta);
                    palabraClave = true;
                }
